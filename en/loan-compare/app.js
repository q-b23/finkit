...
function createLoan() {
  return {
    ...
    name: `Option ${String.fromCharCode(64 + loans.length + 1)}`,
    principal: 0,
    rate: 0,
    termMonths: 12
  };
  ...
  loanList.innerHTML = loans.map(loan => `
    <div class="loan-card">
      <div class="loan-card-header">
        ...
        <button class="btn btn-sm" onclick="removeLoan(${loan.id})">Delete</button>
      </div>
...
function compare() {
  ...
  if (valid.length < 2) {
    alert('Please add at least two complete loan options (amount, APR, and term required)');
    return;
  }
...
function buildResultsHTML() {
  ...
  const recHTML = `
    <div class="recommendation">
      ...
      <div>
        <strong>Recommendation: ${escapeHtml(overallBest.name)}</strong>
        <p style="font-size:0.85rem;color:var(--text-secondary);margin-top:4px;">
          Payment ${formatCurrency(overallBest.monthlyPayment)} · Total ${formatCurrency(overallBest.totalPaid)}，
          Composite Score最优。
          ${loans.length > 1 ? `Max payment difference: ${monthlyDiff} · Max total difference: ${totalDiff}。` : ''}
        </p>
      </div>
    </div>
  `;
  ...
  const breakdownHTML = `
    <div class="chart-section">
      <h3>Total Cost Breakdown (Principal + Interest)</h3>
    ...
    <div class="breakdown-grid">
      ${loans.map(l => `
        <div class="breakdown-card">
          ...
          <div class="breakdown-row"><span>Total Interest</span><span class="br-val">${formatCurrency(l.totalInterest)}</span></div>
...
function drawChart() {
  ...
  ctx.textAlign = 'left';
  ctx.fillText('Principal', pad.left + 26, height - 1);
...
// === Init ===
loans = [
  { id: 1, name: 'Bank A (Mortgage)', principal: 1000000, rate: 3.85, termMonths: 360 },
  { id: 2, name: 'Bank B (Mortgage)', principal: 1000000, rate: 4.10, termMonths: 360 },
  { id: 3, name: 'Bank C (Mortgage)', principal: 1000000, rate: 3.65, termMonths: 360 }
];
render();
